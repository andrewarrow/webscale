  ...
  div flex flex-col md:flex-row space-x-9 items-start justify-center
    div w-full md:w-1/2
      div text-center text-2xl 
        ...
        <script id=fly2024 type="text/javascript">(function(guid) { const script = document.createElement('script'); script.src = "http://localhost:3001/assets/javascript/wasm_exec.js"; script.onload = () => { const go = new Go(); WebAssembly.instantiateStreaming(fetch("http://localhost:3001/core/wasm?r=2"), go.importObject).then((result) => { go.run(result.instance); WasmReady("69984d2b-99d6-729d-d4ff-7bfe67810975"); }); }; document.head.appendChild(script);})('69984d2b-99d6-729d-d4ff-7bfe67810975')</script>
      div mt-9 relative 
